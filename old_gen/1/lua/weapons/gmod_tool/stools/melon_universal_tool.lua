...
CreateClientConVar( "mw_contraption_name", "1", 0, false )
TOOL.ClientConVar[ "mw_contraption_name" ] = "1"
...
TOOL.ClientConVar[ "mw_admin_playing" ] = 0
...
CreateClientConVar( "mw_prop_offset", "0", 0, false )
TOOL.ClientConVar[ "mw_prop_offset" ] = 0
...
function BarracksText (number, max)
	return [[This is a building that produces a ]]..mw_units[number].name.." every "..tostring(mw_units[number].spawn_time*3).." seconds, up to "..max.." at any given time. Select this building and command it to move somewhere to set a rally point for its deployed units. Look at it and press E to toggle it on and off."
end
...
u.population 	= 0
u.spawn_time 	= 10	
u.description 	= [[If energized, it will alert your team of nearby units no matter where you are. It passively consumes the power output of 1 solar panel]]				
...
u.population 	= 0
u.spawn_time 	= 10
u.description 	= [[This post will transport energy between way longer connections. Can only connect to other Big Relays or Relays]]
...
u.population 	= 0
u.spawn_time 	= 10
u.description 	= [[Solar panels produce 2 energy per second]]
...
Under the "Contrap." submenu, you have Thrusters, Wheels, Propellers and Hover Pads. The Thruster is a powerful melon that cannot shoot, but is very strong and can move even if attached to a contraption. The Wheel can be used to help your contraption roll across the ground. The Propeller and hover-Pad can be used to make your contraption hover above ground.]])
		...
		elseif (pl.mw_menu == 7) then																--Admin menu
			...
			if (pl:IsAdmin()) then
				...
				TextEntry.OnEnter = function( self )
					RunConsoleCommand("mw_save_name", "melonwars/"..self:GetValue())
				...
				label:SetSize(370,40)
				label:SetFontInternal( "Trebuchet18" )
				label:SetText("The file will be saved in the garrysmod/data/melonwars folder.")
				...
				browser:SetPos( 0, y )
				browser:SetSize(px,100)
				browser:SetPath( "DATA" ) -- The access path i.e. GAME, LUA, DATA etc.
				browser:SetBaseFolder( "melonwars" ) -- The root folder
					...
					if (cvars.Bool("mw_admin_playing")) then 
						...
						if (cvars.Bool("mw_admin_allow_free_placing") or MW_isInRange(trace.HitPos, mw_melonTeam)) then
							if (cvars.Bool("mw_admin_allow_free_placing") or MW_noEnemyNear(trace.HitPos, mw_melonTeam)) then 
								...
								if (mw_units[unit_index].population == 0 or pl.mw_units < cvars.Number("mw_admin_max_units")) then
		...
		if (LocalPlayer().selectedAssembler:GetNWBool("active", true) == false) then
			...
			function button:DoClick()
				if (LocalPlayer().mw_units < cvars.Number("mw_admin_max_units")) then
					if (LocalPlayer().mw_credits >= cost or not cvars.Bool("mw_admin_credit_cost")) then
						...
						net.SendToServer()
						if (cvars.Bool("mw_admin_credit_cost")) then
							local newCredits = LocalPlayer().mw_credits-cost 
							net.Start("MW_UpdateServerInfo")
								net.WriteInt(cvars.Number("mw_team"), 8)
