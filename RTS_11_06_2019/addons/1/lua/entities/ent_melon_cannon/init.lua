...
function ENT:Initialize()
	...
	self.modelString = "models/props_c17/canister_propane01a.mdl"
...
function ENT:Shoot ( ent, forceTargetPos )
	...
	if (ent:GetVelocity():Length() < 15 && ent.nextShot < CurTime()) then
		...
		if (forceTargetPos != nil or IsValid(ent.targetEntity)) then
			...
			local shootVector = 150*(targetPos-(ent:GetPos()+Vector(0,0,50)) + Vector(0, 0, 30) + Vector(math.random(-self.spread,self.spread),math.random(-self.spread,self.spread),math.random(-self.spread,self.spread)))
