...
function ENT:Think()
	if (self:GetNWInt("mw_melonTeam", -1) == -1) then
		self:SetNWInt("mw_melonTeam", self.mw_melonTeam)
	end
...
function MW_DefaultShoot( ent, forceTargetPos )
	...
	if (forceTargetPos != nil or IsValid(ent.targetEntity)) then
		...
		if (IsValid(ent.targetEntity)) then
			...
			bullet.Dir = (forceTargetPos-pos):GetNormalized()*ent.range*0.99
		end
		bullet.Num=1
		bullet.Src=pos
		...
		effectdata:SetOrigin( pos + bullet.Dir:GetNormalized()*10 )
